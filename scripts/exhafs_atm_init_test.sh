...
${HOMEhafs}/scripts/exhafs_forecast_test.sh
export err=$?; err_chk
...
export DATA=${DATAinit}/atm_merge_post
mkdir -p ${DATA}
cd ${DATA}

${HOMEhafs}/scripts/exhafs_atm_post_test.sh
export err=$?; err_chk
...
export DATA=${DATAinit}/atm_merge_product
mkdir -p ${DATA}
cd ${DATA}
...
for ng in $(seq 1 ${ngrids}); do
  ...
  echo "export neststr=$neststr tilestr=${tilestr} gridstr=${gridstr} lasttile=$(( ${ng} == ${ngrids} )); \
        ${HOMEhafs}/scripts/exhafs_product_test.sh \
