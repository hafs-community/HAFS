...
if [ ${RUN_GSI_VR_ENS} = YES ]; then
  ...
  if [ ${MIX_DA_FLAG} = YES ]; then  # KKUROSAWA
    export RESTARTens_inp_gdas=${WORKhafs}/intercom/mix_ens_recenter_ens
  fi
fi
...
#!/bin/sh
set -x
        ${NCP} ${RESTARTens_inp_gdas}/${tmp_memstr}/${PDY}.${cyc}0000.fv_core.res.tile1.nc fv3sar_tile1_${memstr}_dynvars 
        ${NCP} ${RESTARTens_inp_gdas}/${tmp_memstr}/${PDY}.${cyc}0000.fv_tracer.res.tile1.nc fv3sar_tile1_${memstr}_tracer
EOFprep
    fi
...
if [ $ldo_enscalc_option -eq 0 ]; then # enkf_update
  rm -f cmdfile
#  for memstr in $(seq -f "mem%03g" 1 $nens); do
  for memstr in $(seq -f "mem%03g" 1 $nens_HAFS); do # KKUROSAWA
