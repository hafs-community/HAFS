...
export MERGE_TYPE=${MERGE_TYPE:-vortexreplace}
...
if [ ${MERGE_TYPE} = vortexreplace ]; then
  MERGE_CMD="${DATOOL} vortexreplace --tcvital=${tcvital} --vortexradius=800:900 --relaxzone=50"
elif [ ${MERGE_TYPE} = domainmerge ]; then
