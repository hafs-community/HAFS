...
if [ "${ENSDA}" = YES ]; then
  ...
  FIXgrid=${FIXgrid:-${WORKhafs}/intercom/atm_merge_prep/grid}
  INPdir=${INPdir:-${WORKhafs}/intercom/atm_merge_inp_fgat${FGAT_HR}}
  OUTdir=${OUTdir:-${WORKhafs}/intercom/atm_merge_forecast_init_fgat${FGAT_HR}}
  RESTARTout=${WORKhafs}/intercom/RESTART_atm_merge_init_fgat${FGAT_HR}
  intercompost=${WORKhafs}/intercom/atm_merge_init_fgat${FGAT_HR}/post
  intercomocnpost=${WORKhafs}/intercom/atm_merge_init_fgat${FGAT_HR}/ocn_post
  intercomgempak=${WORKhafs}/intercom/atm_merge_init_fgat${FGAT_HR}/gempak
else
  FIXgrid=${FIXgrid:-${WORKhafs}/intercom/atm_merge_prep/grid}
  INPdir=${INPdir:-${WORKhafs}/intercom/atm_merge_inp}
  OUTdir=${OUTdir:-${WORKhafs}/intercom/atm_merge_forecast_init}
  RESTARTout=${WORKhafs}/intercom/RESTART_atm_merge_init
  intercompost=${WORKhafs}/intercom/atm_merge_init/post
  intercomocnpost=${WORKhafs}/intercom/atm_merge_init/ocn_post
  intercomgempak=${WORKhafs}/intercom/atm_merge_init/gempak
fi
#NHRS=$(awk "BEGIN {print ${dt_atmos}/3600}")
#NHRS_ENS=$(awk "BEGIN {print ${dt_atmos}/3600}")
#restart_interval="$(awk "BEGIN {print ${dt_atmos}/3600}") 6"
NHRS=0.02
restart_interval="0.02 6"
...
for itile in $(seq 8 ${ntiles}); do
  ...
  # Link static files for nest initialization 
  if [[ "${is_moving_nest}" = *".true."* ]] || [[ "${is_moving_nest}" = *".T."* ]]; then
    for var in facsf maximum_snow_albedo slope_type snowfree_albedo soil_type substrate_temperature vegetation_greenness vegetation_type; do
...
if [[ "${is_moving_nest}" = *".true."* ]] || [[ "${is_moving_nest}" = *".T."* ]]; then
  ...
  # Added parent soil_type to properly handle lakes
...
io_layoutx_nml=$( echo ${io_layoutx} | cut -d , -f ${n} )
io_layouty_nml=$( echo ${io_layouty} | cut -d , -f ${n} )
npx_nml=901 #1201 #$( echo ${npx} | cut -d , -f ${n} )
npy_nml=901 #1201 #$( echo ${npy} | cut -d , -f ${n} )
k_split_nml=$( echo ${k_split} | cut -d , -f ${n} )
n_split_nml=$( echo ${n_split} | cut -d , -f ${n} )
hord_mt_nml=$( echo ${hord_mt} | cut -d , -f ${n} )
hord_vt_nml=$( echo ${hord_vt} | cut -d , -f ${n} )
...
iau_regional_nml=$( echo ${iau_regional} | cut -d , -f ${n} )
iau_inc_scale_nml=$( echo ${iau_inc_scale} | cut -d , -f ${n} )
