...
if [ "${ENSDA}" = YES ]; then
  ...
  INPdir=${WORKhafs}/intercom/atm_merge_forecast_init_fgat${FGAT_HR}
  COMOUTpost=${WORKhafs}/intercom/atm_merge_init_fgat${FGAT_HR}
  intercom=${WORKhafs}/intercom/atm_merge_init_fgat${FGAT_HR}/post
  NHRS=0
else
  INPdir=${WORKhafs}/intercom/atm_merge_forecast_init
  COMOUTpost=${WORKhafs}/intercom/atm_merge_init
  intercom=${WORKhafs}/intercom/atm_merge_init/post
  NHRS=0
fi
  ...
  # for file_res in $fv_core_tile $fv_tracer_tile $fv_srf_wnd_tile $sfc_data $phy_data ; do
    for file_res in $fv_core_tile $fv_tracer_tile $fv_srf_wnd_tile $sfc_data ; do
      if [ -s ${INPdir}/RESTART/${file_res} ] && [ ${INPdir}/RESTART/${file_res} -nt ${RESTARTcom}/${file_res} ]; then
        if [ "${ENSDA}" = YES ]; then
          echo ncks -4 --deflate=1 -O ${INPdir}/RESTART/${file_res} ${RESTARTcom}/${file_res} >> cmdfile
        else
