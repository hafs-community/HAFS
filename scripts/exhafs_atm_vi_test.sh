...
if [[ ${vi_domain} == 40 ]]; then
 deg_box1=30
 deg_box2=45
if [[ ${vi_domain} == 30 ]]; then
 #NOT tested yet Not supported yet. DON'T use 35 by 35
 deg_box1=35
 deg_box2=35
fi
elif [[ ${vi_domain} == 20 ]]; then
 deg_box1=25
 deg_box2=25
fi
...
#for vortexradius in ${deg_box1} ${deg_box2}; do
for res in 0.02 0.20; do
  if [[ ${res} == 0.02 ]]; then
    vortexradius=${deg_box1}
  elif [[ ${res} == 0.20 ]]; then
