...
if [ ${ENSDA:-NO} = YES ]; then
  ...
  export CASE=C512
  export istart_nest=213
  export jstart_nest=213
  export iend_nest=812
  export jend_nest=812
  export idim_nest=900
  export jdim_nest=900
...
if [ ${FGAT_MODEL} = gdas ]; then
 export OUTDIR=${OUTDIR:-${WORKhafs}/intercom/atm_merge_prep_fgat${FGAT_HR}/grid/${CASE}}
 export DATA=${DATA:-${WORKhafs}/atm_merge_prep_fgat${FGAT_HR}}
else
 export OUTDIR=${OUTDIR:-${WORKhafs}/intercom/atm_merge_prep/grid/${CASE}}
 export DATA=${DATA:-${WORKhafs}/atm_merge_prep}
fi
#  export OUTDIR=${OUTDIR:-${WORKhafs}/intercom/atm_merge_prep/grid/${CASE}}
fi
...
if [[ "${is_moving_nest}" = *".true."* ]]; then
  export CASE=C512
  export istart_nest=273
  export jstart_nest=273
  export iend_nest=752
  export jend_nest=752
  export idim_nest=2160
  export jdim_nest=2160
  ...
  export OUTDIR=${WORKhafs}/intercom/atm_merge_prep_mvnest/grid_mvnest1res/${CASE}
...
if [ ${FGAT_MODEL} = gdas ]; then
 export out_dir=${OUTDIR:-${WORKhafs}/intercom/atm_merge_prep_fgat${FGAT_HR}/grid}
 export DATA=${DATA:-${WORKhafs}/atm_merge_prep_fgat${FGAT_HR}}
else
 export out_dir=${OUTDIR:-${WORKhafs}/intercom/atm_merge_prep/grid}
 export DATA=${DATA:-${WORKhafs}/atm_merge_prep}
fi
...
if [[ ${ATM_PREP_MVNEST:-NO} = YES ]] && [[ "${is_moving_nest}" = *".true."* ]]; then
  mkdir -p $gridfixdir
  if [ -d $gridfixdir ]; then
    echo "Copy the grid fix files to gridfixdir $gridfixdir cache for reuse in later cycles"
    ${NCP} -p -r ${out_dir}/* $gridfixdir/
    ls ${gridfixdir}
  else
    echo "WARNING: UNABLE to copy grid fix files for reuse in later cycles, because gridfixdir does not exist: $gridfixdir"
  fi
fi
# LJG
