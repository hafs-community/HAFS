...
# Run one cycle of a storm with C96
 ${PYTHON3} ./run_hafs.py -t ${dev} 2020082512 13L HISTORY \
    config.EXPT=${EXPT} config.SUBEXPT=${EXPT}_globmovenest_C96_Laura13L \
    config.NHRS=126 ${scrubopt} \
    ../parm/hafs_globmovenest_C96_Laura13L_2020082512.conf

# Run one cycle of a storm with C768
 ${PYTHON3} ./run_hafs.py -t ${dev} 2020082512 13L HISTORY \
     config.EXPT=${EXPT} config.SUBEXPT=${EXPT}_globmovenest_C768_Laura13L \
     config.NHRS=126 ${scrubopt} \
     ../parm/hafs_globmovenest_C768_Laura13L_2020082512.conf
