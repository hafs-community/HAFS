...
${PYTHON3} ./run_hafs.py -t ${dev} 2019092600 13L HISTORY \
    config.EXPT=${EXPT} config.SUBEXPT=hafs_movingnest_C96 \
    config.NHRS=36 ${scrubopt} \
    ../parm/hafs_globmovenest_C96.conf

# Run one cycle of a storm with C768
${PYTHON3} ./run_hafs.py -t ${dev} 2019092600 13L HISTORY \
    config.EXPT=${EXPT} config.SUBEXPT=hafs_movingnest_C768 \
    config.NHRS=36 ${scrubopt} \
    ../parm/hafs_globmovenest_C768.conf
