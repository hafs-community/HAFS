...
static int unpack_double(grib_accessor* a, double* val, size_t *len)
{
    ...
    sec_val = (long*)grib_context_malloc(gh->context,(n_vals)*sizeof(long));
    ...
    if(orderOfSpatialDifferencing){
        ...
        if (orderOfSpatialDifferencing != 1 && orderOfSpatialDifferencing != 2) {
            grib_context_log(gh->context,GRIB_LOG_ERROR, "Unsupported order of spatial differencing %ld", orderOfSpatialDifferencing);
        ...
        post_process(gh->context, sec_val, n_vals, orderOfSpatialDifferencing, bias, extras);
