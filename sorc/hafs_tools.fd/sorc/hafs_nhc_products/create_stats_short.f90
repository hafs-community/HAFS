...
!================================================================
  ...
  !---parameter define
  character*80 atcf,storm_name,storm_model,skip,lower_case,lower,upper
  ...
  parameter(nmax=200)
  integer :: i,j,k
  real, dimension(0:nmax) :: fhr,wind,press,rlat,rlon,direction,speed
  character clat(0:nmax)*1,clon(0:nmax)*1
  ...
  integer iscolt(3),iecolt(3),istt,imm

  data head1/'ATTENTION...NATIONAL HURRICANE CENTER'/,             &
       head2/'NCEP HAFS HURRICANE MODEL FORECAST MADE FOR'/,       &
       head3/'INITIAL TIME        '/,                              &
  ...
  read(10,'(A)')storm_model
  close(10)
  head2='NCEP '//trim(storm_model)//' HURRICANE MODEL FORECAST MADE FOR'
  ...
  k=0
  do i=1,nmax
    read(20,'(29x,I4,1x,I4,A1,1x,I5,A1,1x,I4,1x,I5)',end=101,err=9991) tmp1,tmp2,s1,tmp3,s2,tmp4,tmp5
  ...
  j=1
  read(19,'(29x,I4,1x,I4,A1,1x,I5,A1,1x,I4,1x,I5)',err=910) tmp1,tmp2,s1,tmp3,s2,tmp4,tmp5
  j=0
  ...
  do i=j+1,k
    ...
    direction(i)=atan2d(dlon,dlat)
    if(direction(i) .lt. 0.0) direction(i)=direction(i)+360.0
    !---in case we do not have previous atcf, assume storm heading/speed same as i=2
    if (j == 1 .and. i ==2 ) write(51,4050) ifix(fhr(1)),rlat(1),-rlon(1),direction(2),'/',speed(2)
    !---write(*,*)'dlat1,dlat2,dlon1,dlon2,distance=',dlat1,dlat2,dlon1,dlon2,dis,direction(i),speed(i)
    write(51,4050) ifix(fhr(i)),rlat(i),-rlon(i),direction(i),'/',speed(i)
  enddo
