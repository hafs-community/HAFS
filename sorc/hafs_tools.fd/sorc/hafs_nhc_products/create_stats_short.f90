...
!atcf=matthew14l.2016100706.trak.nmmb.atcfunix
         ! construct atcf file name
         atcf=lower(trim(storm_name)//trim(storm_id)//'.'  &
     &        //yyyy//mm//dd//hh//'.hafs.trak.atcfunix')
          write(*,*)'atcf=',trim(atcf)

!AL, 14, 2016100706, 03, HAFS, 000, 277N,  797W, 103,  938, XX,  34, NEQ, 023 
          do i=0,nmax
            fhr(i)=-999.0
          enddo
          
!          open(20,file=trim(atcf),status='old')
            k=0
           do i=1,nmax
 ...
 !             write(*,*)'tmp1,fhr(i-1)=',tmp1,fhr(i-1)
 !           write(*,*)'i=,hr=',i,tmp1, tmp2,s1,tmp3,s2,tmp4,tmp5
             k=k+1
             fhr(k)=float(tmp1)
...
101        close(20)
          ...
          !write stats.shot file
!HOUR:126.0  LONG:  -79.25  LAT:  16.51  MIN PRESS (hPa):  940.02   MAX SURF WIND (KNOTS):103.50
           do i=1,k
 !           write(*,*)i,fhr(i),rlat(i),rlon(i),wind(i),press(i)
...
4050      format(1x,i3,9X,F7.1,10X,F7.1,10x,f5.0,a,f4.1)
           ...
           close(51)
!!!!!!!!!!!!!! write tpc file !!!!!!!!!!!!!!!!!!!!!!
         ! read first 8 line from fort.51, then write to 61
          do i=1,8
          read(51,'(A)')skip
          write(*,*)skip
          write(61,9)skip
