...
_hafs_nexrad_superob (){
    ...
    # Generate makefile using CMake for the application
    cmake ../hafs_nexrad_superob -DCMAKE_Fortran_COMPILER=${CMAKE_Fortran_COMPILER} -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER} -DBUILD_TYPE=${BUILD_TYPE}

    # Build the hafs_nexrad_superob application.
    make all VERBOSE=3
...
_hafsenmean_recenter (){
    ...
    # Build the hafs_enmean_recenter application.
    make all VERBOSE=3

    # Move the hafs_enmean_recenter application executable to the HAFS
    # utility application executables path.
    make install
}
#----
