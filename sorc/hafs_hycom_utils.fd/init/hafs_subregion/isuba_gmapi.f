...
c     on the target grid, i.e. abs(x_out(i+1,j)-x_out(i,j)) and
c     abs(y_out(i+1,j)-y_out(i,j)) must both be less than maxinc (after
...
c
      ...
      integer              :: i,ii,ios,ip,iq,ix,itmp,j,jj,jp,jq,l,
     &                        ni,nir,no,nor
      integer              :: k,l0,l1
...
c           
c           find the first point by exhaustive search.
c
            ip = 1
            jp = 1
            if = 1
...
c           
c           then assume result is "close" to the nearest target point.
c
            if     (ii.eq.1) then
              ip = i_out(ii,jj-1)
              jp = j_out(ii,jj-1)
